...
const YouTubeLightPlayer = ({ programImage, id, title }: { programImage: ImagesNM, id: string, title: string }) => {
  ...
  let tag = (<div className={'fade-in'}>
      ...
      <div className={styles.youtubePlayerIconContainer} onClick={() => setIsPlaying(true)}>
        <Image src={'/icons/play-button.svg'} alt='play-button' width={80} height={80} loading={'eager'} style={{ backgroundColor: 'white', borderRadius: '50%'}}/>
      </div>
  </div>
  );
